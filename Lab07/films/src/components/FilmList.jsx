...
function FilmList(props) {
    ...
    return (<ListGroup id="films-list" variant="flush">
        {films.map((film) => <FilmInList filmData={film} key={film.id} setFormVisible={props.setFormVisible} setEditableFilm={props.setEditableFilm}/>)}
    </ListGroup>);
}
            ...
            <Col xs={8} xl={3} className="actions-container text-end">
                ...
                <div className="d-none d-xl-flex actions">
                    <i className="bi bi-pencil" onClick={()=>{
                        props.setFormVisible('edit');
                        props.setEditableFilm(filmData);
                    }}></i>
                    <i className="bi bi-trash"></i>
                </div>
            </Col>
