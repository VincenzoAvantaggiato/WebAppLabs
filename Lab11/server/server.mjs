...
app.get('/api/films',isLoggedIn,
    (req, res) => {
        ...
        filmDao.getFilms(req.query.filter,req.user.id)
...
app.get('/api/films/:id',isLoggedIn,
    async (req, res) => {
        try {
            const result = await filmDao.getFilm(req.params.id,req.user.id);
            if (result.error)
...
app.put('/api/films/:id',isLoggedIn,
    ...
    async (req, res) => {
        ...
        const film = new Film(Number(req.params.id), req.body.title, req.body.favorite, req.body.watchDate, req.body.rating, req.user.id);

        try {
            const result = await filmDao.updateFilm(film.id, film,req.user.id);
            if (result.error)
                res.status(404).json(result);
            else
                res.json(result);
